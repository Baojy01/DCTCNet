...
class MyAPSeDCTConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        Q_quarter = G @ torch.exp(self.Se_weight) @ G.transpose(1, 2)
        ...
        for i in range(N):
            for u in range(N):
                if u == 0:
                    ...
                    T[i, u] = ((N - i) * math.cos(pi * u * i / N) - math.sin(pi * u * i / N) / math.sin(pi * u / N)) / (N ** 2)
...
class MultiAPSeDCT(nn.Module):
    """
        ...
        kernel_size: the convolution kernel_size (such as 1,3,5,7); It can have two forms:
                    ...
                    3. if kernel_size=1, we adopt 3*3 depthwise convolution.
    """
    ...
    def __init__(self, in_channels: int, out_channels: int, kernel_size: Union[int, Dict[int, int]], groups=1):
        ...
        for cur_kernel_size, cur_heads in self.kernel_size.items():
            if cur_kernel_size == 1:
                cur_conv = nn.Conv2d(cur_heads * Ch, cur_heads * Ch, kernel_size=3, groups=cur_heads * Ch, bias=False)
            else:
