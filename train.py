...
parser.add_argument('--dataset', default='imagenet1k', type=str, choices=['imagenet1k', 'cifar10', 'cifar100'], help='dataset for training')
parser.add_argument('--num_classes', default=1000, type=int, help='number of classes for classification')
parser.add_argument('--data_dir', default='../data/ImageNet1K', type=str, help='path of dataset')
parser.add_argument('--arch', default='MultiAPSeDCTNet50', type=str, choices=model_names, metavar='ARCH', help='model architecture (default: resnet50)')
...
parser.add_argument('--ratio', default=0, type=float, help='The ratio of APSeDCTConv channels')
...
parser.add_argument('--batch_size', default=256, type=int, metavar='BS', help='mini-batch size (default: 256)')
...
parser.add_argument("--amp", default=True, type=bool, help="Use torch.cuda.amp for mixed precision training (default: True)")
...
def main():
    ...
    train_set, val_set = GetData(args.dataset, args.data_dir)
    ...
    model = get_model(args.arch, args.num_classes, args.kernel_size, args.ratio)
    model = model.to(device)

    results_dir = './results/%s/' % (args.arch + '_' + args.dataset + '_bs_' + str(args.batch_size) + '_k_' + str(args.kernel_size) + '_epochs_' + str(args.epochs) + '_ratio_' + str(args.ratio))
    save_dir = './checkpoint/%s/' % (args.arch + '_' + args.dataset + '_bs_' + str(args.batch_size) + '_k_' + str(args.kernel_size) + '_epochs_' + str(args.epochs) + '_ratio_' + str(args.ratio))
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            checkpoint = torch.load(args.resume_dir, map_location=args.device)
